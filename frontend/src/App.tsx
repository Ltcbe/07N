...
import { fetchMultipleStations, MAIN_STATIONS } from './lib/irailApi'
...
export default function App() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await fetchMultipleStations(MAIN_STATIONS, selectedDate)
        if (!cancelled) setTrains(data)
      } catch (e) {
        console.error(e)
        if (!cancelled) setError('Impossible de charger les données IRAIL')
      } finally {
        ...
        {!loading && !error && (
          <>
            ...
            <div className="mt-4" style={{color:'#64748b', fontSize: 12}}>
              Données fournies par l’API IRAIL — Dernière mise à jour : {new Date().toLocaleTimeString('fr-BE')}
            </div>
          </>
        )}
      </main>
