...
export const HourlyChart: React.FC<{trains: Train[]}> = ({trains}) => {
  const data = useMemo(()=>{
    ...
    for (const t of trains) {
      ...
      buckets[h].avgDelay += (t.delay || 0)
      buckets[h].count += 1
    }
    return Object.values(buckets).sort((a,b)=>a.hour.localeCompare(b.hour))
      .map(b=>({hour:b.hour, avgDelay: b.count? +(b.avgDelay/b.count).toFixed(1) : 0}))
  }, [trains])

  return (
    <div className="card" style={{height:'320px'}}>
      <div className="mb-4" style={{color:'#334155'}}>Retard moyen par heure</div>
