...
export const FilterBar: React.FC<FilterBarProps> = ({
  ...
  const sorted = [...BELGIAN_STATIONS].sort((a,b)=> a.localeCompare(b,'fr',{sensitivity:'base'}))
    ...
    <div className="card">
      <div className="grid" style={{gridTemplateColumns:'1fr 1fr 1fr', gap: '12px'}}>
        <div>
          ...
          <input className="input w-full" type="date" value={dateStr} onChange={(e)=>onDateChange(new Date(e.target.value+'T00:00:00'))} />
        </div>
        <div>
          <label className="block mb-2 text-sm text-slate-600">Gare de départ</label>
          <select className="input w-full" value={departureStation} onChange={(e)=>onDepartureChange(e.target.value)}>
        ...
        <div>
          ...
          <select className="input w-full" value={arrivalStation} onChange={(e)=>onArrivalChange(e.target.value)}>
            <option value="">-- Sélectionner une gare --</option>
            {sorted.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
